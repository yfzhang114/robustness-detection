...
def sample_from_model(texts, min_words=55, prompt_tokens=30):
    ...
    if args.dataset == 'pubmed':
        ...
        if 'gpt' in args.base_model_name or 'vicuna' in args.base_model_name:
    ...
    if args.openai_model:
        ...
        tries = 0
        while (m := min(len(x.split()) for x in decoded)) < min_words and 'vicuna' not in args.base_model_name:
            if tries != 0:
                print()
            ...
            with torch.no_grad():
                if 'vicuna' in args.base_model_name:
                    print('generated data by vicuna model')
                    temperature=0.7
                    outputs = base_model.generate(**all_encoded, do_sample=True if temperature > 1e-5 else False,
...
def get_prob_margin(text):
    ...
    if args.openai_model:        
        ...
        with torch.no_grad():
            ...
            logits = logits[:-1]
            tokens = labels.squeeze()[1:]
            probs = F.softmax(logits, dim=1)
    
    input_probs = torch.gather(probs, dim=1, index=tokens.unsqueeze(1))
    max_probs, max_indexes = torch.max(probs, dim=1)
    prob_margin = max_probs - input_probs.squeeze()
...
def get_perturbation_results(span_length=10, n_perturbations=1, n_samples=500):
    ...
    original_text2 = data2["original"]
    sampled_text2 = data2["sampled"]

    perturb_fn = functools.partial(perturb_texts, span_length=span_length, pct=args.pct_words_masked)

    p_original_text = perturb_fn([x for x in original_text for _ in range(n_perturbations)])
    p_sampled_text = perturb_fn([x for x in sampled_text2 for _ in range(n_perturbations)])
    ...
    assert len(p_sampled_text) == len(sampled_text2) * n_perturbations, f"Expected {len(sampled_text2) * n_perturbations} perturbed samples, got {len(p_sampled_text)}"
    assert len(p_original_text) == len(original_text) * n_perturbations, f"Expected {len(original_text) * n_perturbations} perturbed samples, got {len(p_original_text)}"

    for idx in range(len(original_text)):
        results.append({
            "original": original_text[idx],
            "sampled": sampled_text2[idx],
...
def run_perturbation_experiment(results, criterion, span_length=10, n_perturbations=1, n_samples=500):
    ...
    result90 = get_native_detection_metric(np.array(predictions['real']), np.array(predictions['samples']), ratio=0.9)
    result = get_native_detection_metric(np.array(predictions['real']), np.array(predictions['samples']))
    result99 = get_native_detection_metric(np.array(predictions['real']), np.array(predictions['samples']), ratio=0.99)
    ...
    return {
        ...
        'ood detection 90': result90,
        'ood detection 99': result99,
        'loss': 1 - pr_auc,
    }
...
def run_baseline_threshold_experiment(criterion_fn, name, n_samples=500):
    ...
    for batch in tqdm.tqdm(range(n_samples // batch_size), desc=f"Computing {name} criterion"):
        ...
        sampled_text = data2["sampled"][batch * batch_size:(batch + 1) * batch_size]
    ...
    result99 = get_native_detection_metric(np.array(predictions['real']), np.array(predictions['samples']), ratio=0.99)
    p, r, pr_auc = get_precision_recall_metrics(predictions['real'], predictions['samples'])
    print(f"{name}_threshold ROC AUC: {roc_auc}, PR AUC: {pr_auc} fpr90: {result90['FPR']}, fpr95: {result['FPR']}, fpr99: {result99['FPR']}")
    
    plt_vector_difference(predictions['real'], predictions['samples'], f'{name}_{n_samples}.jpg')
    return {
...
def generate_samples(raw_data, batch_size, min_words):
    ...
    for batch in range(len(raw_data) // batch_size):
        ...
        sampled_text = sample_from_model(original_text, min_words)
...
def generate_data(dataset, key):
    ...
    min_words=30 if dataset in ['pubmed', 'code'] else 55

    return generate_samples(data[:n_samples], batch_size=batch_size, min_words=min_words)
...
def eval_supervised(data, model):
    ...
    print(f"{model} ROC AUC: {roc_auc}, PR AUC: {pr_auc} fpr90: {result90['FPR']}, fpr95: {result['FPR']}, fpr99: {result99['FPR']}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset2', type=str, default="code")
    ...
    if 'gpt' not in args.base_model_name:
        args.batch_size = args.batch_size // 5
    ...
    if 'gpt' in args.base_model_name:
    ...
    elif 'alpaca' in args.base_model_name:
        ...
        base_tokenizer.return_token_type_ids=False
        preproc_tokenizer = base_tokenizer
    elif 'vicuna' in args.base_model_name:
        from fastchat.model import load_model
        print("Setting up Vicuna model: ", args.base_model_name)
        ...
        # base_tokenizer.pad_token_id = base_model.config.eos_token_id
        base_model.eval()
        print("Finished")
    ...
    if args.dataset == 'squad':
        args.dataset_key = 'context'
    data = generate_data(args.dataset, args.dataset_key)
    
    print(f'Loading dataset2 {args.dataset2}...')
    if args.dataset2 == 'squad':
        args.dataset_key = 'context'
    if args.dataset == args.dataset2:
        data2 = data
    else:
        data2 = generate_data(args.dataset2, args.dataset_key)
    ...
    if not args.skip_baselines:
        baseline_outputs = []
        # baseline_outputs.append(run_baseline_threshold_experiment(get_prob_margin, "probability margin", n_samples=n_samples))
        baseline_outputs.append(run_baseline_threshold_experiment(get_ll, "likelihood", n_samples=n_samples))
        if args.openai_model is None:
